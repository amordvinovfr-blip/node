...
export const ABUSE_BLOCKER_RECON_PORTS = [
...
const HorizontalSweepRuleSchema = z
    .object({
        enabled: z.boolean().default(true),
        windowSeconds: z.int().min(60).max(3600).default(900),
        uniqueNetworks: z.int().min(2).max(4096).default(150),
        ipv4Prefix: z.int().min(8).max(32).default(24),
        ipv6Prefix: z.int().min(16).max(128).default(48),
        maxNetworksPerKey: z.int().min(2).max(4096).default(256),
        score: z.int().min(1).max(10000).default(100),
        blockEligible: z.boolean().default(true),
    })
    ...
    .meta({
        ...
        markdownDescription: abuseBlockerDocs(
            'Distinct destination networks (IPv4 /24, IPv6 /48) per user and scan port over the window.',
        ),
    });

const HammerTargetRuleSchema = z
    .object({
        enabled: z.boolean().default(true),
        windowSeconds: z.int().min(60).max(3600).default(900),
        sessions: z.int().min(2).max(65535).default(300),
        maxTargetsPerUser: z.int().min(1).max(4096).default(64),
        score: z.int().min(1).max(10000).default(100),
        blockEligible: z.boolean().default(true),
    })
    .prefault({})
    .meta({
        title: 'Hammer Target',
        markdownDescription: abuseBlockerDocs(
            'Sessions per user to one destination (IP or hostname) and scan port over the window.',
        ),
    });

const SessionRateBurstRuleSchema = z
    .object({
        enabled: z.boolean().default(true),
        windowSeconds: z.int().min(5).max(3600).default(60),
        sessions: z.int().min(2).max(1000000).default(600),
        score: z.int().min(1).max(10000).default(100),
        blockEligible: z.boolean().default(false),
    })
    .prefault({})
    .meta({
        title: 'Session Rate Burst',
        markdownDescription: abuseBlockerDocs(
            'Sessions per user on any port outside excludedPorts over the window. Report-only by default.',
        ),
    });
...
const DomainRulesSchema = z
    .object({
        ...
        sweep: z
            .object({
                enabled: z.boolean().default(true),
                windowSeconds: z.int().min(60).max(3600).default(900),
                uniqueDomains: z.int().min(2).max(4096).default(50),
                maxDomainsPerKey: z.int().min(2).max(4096).default(256),
                score: z.int().min(1).max(10000).default(100),
                blockEligible: z.boolean().default(true),
            })
        ...
        subdomainSweep: z
            .object({
                enabled: z.boolean().default(true),
                windowSeconds: z.int().min(60).max(3600).default(900),
                uniqueHosts: z.int().min(2).max(4096).default(100),
                maxHostsPerKey: z.int().min(2).max(4096).default(256),
                maxDomainsPerUser: z.int().min(1).max(4096).default(32),
                score: z.int().min(1).max(10000).default(100),
                blockEligible: z.boolean().default(false),
            })
    ...
    .meta({
        ...
        markdownDescription: abuseBlockerDocs(
            'Scores requests by hostname on scan ports: per-hostname hammer, distinct registrable domains, and distinct hostnames under one domain. Never resolves DNS.',
        ),
    });

const AbuseBlockerSourceGuardsSchema = z
    .object({
        enabled: z.boolean().default(true),
        blockNonPublicSources: z.boolean().default(false),
        maxUsersPerSource: z.int().min(1).max(16).default(1),
        userWindowSeconds: z.int().min(60).max(86400).default(3600),
        maxTrackedSources: z.int().min(1).max(1000000).default(100000),
        reportOnlyUserIds: z.array(z.int().min(1)).default([]),
        reportOnlyInboundTags: z.array(z.string().min(1)).default([]),
    })
    .prefault({})
    .meta({
        title: 'Source Guards',
        markdownDescription: abuseBlockerDocs(
            'Turns a block into a report for non-public sources, listed users or inbounds, and sources shared by more than maxUsersPerSource users.',
        ),
    });
    ...
    .object({
        ...
        mode: z
            ...
            .meta({
                title: 'Mode',
                markdownDescription: abuseBlockerDocs(
                    'report: decisions are reported only. block: source IPs of confirmed incidents are blocked.',
                ),
            }),
        ruleSet: z
            .enum(['v2', 'legacy', 'both'])
            ...
            .meta({
                title: 'Rule Set',
                markdownDescription: abuseBlockerDocs(
                    'v2: horizontalSweep, hammerTarget, sessionRateBurst and domain rules. legacy: horizontalScan and destinationSweep. both: all of them.',
                ),
            }),
        scanPorts: z
            .array(z.int().min(1).max(65535))
            ...
            .default(ABUSE_BLOCKER_RECON_PORTS)
            .meta({
                title: 'Scan Ports',
                markdownDescription: abuseBlockerDocs(
                    'Only these destination ports feed scan and hammer rules. Empty: every port not in excludedPorts.',
                ),
            }),
        confirmationSeconds: z.int().min(0).max(3600).default(60),
        rearmAfterCooldown: z.boolean().default(true),
